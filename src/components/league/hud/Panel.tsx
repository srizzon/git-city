 ...
 * phones. Esc and the close button dismiss it.
 */
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-40 flex max-h-[78vh] sm:inset-y-4 sm:right-4 sm:left-auto sm:max-h-none sm:w-[400px]">
      <section
        role="dialog"
        aria-label={title}
        className={`${HUD_BOX} flex w-full flex-col bg-bg/90 font-pixel uppercase text-warm`}
      >
        <header className="flex items-center justify-between border-b-2 border-border px-4 py-3">
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="btn-press -mr-1 px-2 py-1 text-xs text-muted transition-colors hover:text-cream"
          >
            ✕
          </button>
        </header>
