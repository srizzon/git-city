...
// - A bump knocks the crown loose (it lands a few meters off and can be
//   picked up after a moment); a bump with boost steals it outright; an
//   attack that catches the holder knocks it loose too.
...
export const CROWN = {
  ...
  reach: 3.5,
  /** A knocked-off crown can't be picked up for this long (ms). */
  loose: 700,
  immune: 1_500,
...
export function idleCrown(): CrownState {
  return { phase: "idle", startsAt: 0, endsAt: 0, holder: null, x: 0, z: 0, takeAt: 0, since: 0, counted: 0, left: {}, winner: null };
}
...
/** The holder loses the crown; it lands at (x, z). */
export function dropCrown(s: CrownState, now: number, x: number, z: number): boolean {
...
/** Where a knocked-off crown lands: a few meters off the holder, pseudo-random. */
