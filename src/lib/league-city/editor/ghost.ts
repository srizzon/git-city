...
import { LOCKED_TEXT, addProblem, lotObjectAt, type EditorState, type Spot } from "./state";
...
function propsNearLot(s: Pick<EditorState, "objects" | "h">, x: number, z: number): boolean {
  ...
  for (const o of s.objects.values()) {
    ...
    if (propProblem(probe.values(), s.h, { item_type: o.item_type, px: o.px, pz: o.pz, id: o.id }) === "on_building") return true;
  }
  return false;
}

function propFit(s: Pick<EditorState, "objects" | "h">, item: ItemType, spot: Spot, id?: string): GhostFit {
  const px = snap(spot.wx, spot.free);
  const pz = snap(spot.wz, spot.free);
  const problem = propProblem(s.objects.values(), s.h, { item_type: item, px, pz, id });
  return problem ? { ok: false, reason: PROP_PROBLEM_TEXT[problem], px, pz } : { ok: true, px, pz };
}

function surfaceFit(s: Pick<EditorState, "objects" | "h">, item: ItemType, spot: Spot): GhostFit {
  if (!inBounds(s.h, spot.x, spot.z)) return { ok: false, reason: "That's outside the city." };
  const there = lotObjectAt(s.objects, spot.x, spot.z);
  if (there?.kind === "building") return { ok: false, reason: "A building is there." };
  if (there?.locked) return { ok: false, reason: LOCKED_TEXT };
  if (there && there.item_type !== item) return { ok: true, replaces: there };
  return { ok: true };
}

export function ghostFit(s: Pick<EditorState, "objects" | "h" | "held" | "tool" | "hasLogo">, spot: Spot): GhostFit | null {
  ...
  if (s.tool.kind === "place") {
    const blocked = addProblem(s, s.tool.item);
